...
public class StrainView extends MapView
{
	...
	public Map<String, String> getSummaryData(int x, int y)
	{
		...
		values.put("XX", fmt(data.strain()[0]));
		values.put("YY", fmt(data.strain()[1]));
		values.put("ZZ", fmt(data.strain()[2]));
		values.put("XY", fmt(data.strain()[3]));
		values.put("XZ", fmt(data.strain()[4]));
		values.put("YZ", fmt(data.strain()[5]));
